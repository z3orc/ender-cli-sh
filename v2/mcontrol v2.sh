...
setup() {
        ...
        rm .mserversettings 2>/dev/null
        touch .mserversettings
        ...
        echo DIR="${DIR// /}" >> $DIR/.mserversettings | xargs
        # echo VERSION="${VERSION// /}" >> $DIR/.mserversettings | xargs
        echo FLAVOUR="${FLAVOUR// /}" >> $DIR/.mserversettings | xargs
        echo RAM="${RAM// /}" >> $DIR/.mserversettings | xargs
        echo PLAYER_COUNT="${PLAYER_COUNT// /}" >> $DIR/.mserversettings | xargs
        echo WORLD_SEED="${WORLD_SEED// /}" >> $DIR/.mserversettings | xargs
        echo PORT="${PORT// /}" >> $DIR/.mserversettings | xargs
        echo GAMEMODE="${GAMEMODE// /}" >> $DIR/.mserversettings | xargs
        echo DIFFICULTY="${DIFFICULTY// /}" >> $DIR/.mserversettings | xargs
        echo WHITELIST="${WHITELIST// /}" >> $DIR/.mserversettings | xargs
        echo JAR="server.jar" >> $DIR/.mserversettings | xargs
        echo RCONPASS="${RCONPASS// /}" >> $DIR/.mserversettings | xargs
        ...
        #     echo "[  $(tput setaf 3).....$(tput sgr 0)  ] Whitelist value changed"

        # else
        #     echo "[  $(tput setaf 3).....$(tput sgr 0)  ] Whitelist value not changed"
...
# start(){
#         source .mserversettings

#         cd $DIR/serverfiles

#         echo "[  $(tput setaf 3).....$(tput sgr 0)  ] Starting server!"

#         tmux new -d -s minecraft $DIR/bin/jdk-16.0.1+9-jre/bin/java -Xms$RAM -Xmx$RAM -XX:+UseG1GC -XX:+ParallelRefProcEnabled -XX:MaxGCPauseMillis=200 -XX:+UnlockExperimentalVMOptions -XX:+DisableExplicitGC -XX:+AlwaysPreTouch -XX:G1NewSizePercent=30 -XX:G1MaxNewSizePercent=40 -XX:G1HeapRegionSize=8M -XX:G1ReservePercent=20 -XX:G1HeapWastePercent=5 -XX:G1MixedGCCountTarget=4 -XX:InitiatingHeapOccupancyPercent=15 -XX:G1MixedGCLiveThresholdPercent=90 -XX:G1RSetUpdatingPauseTimePercent=5 -XX:SurvivorRatio=32 -XX:+PerfDisableSharedMem -XX:MaxTenuringThreshold=1 -Dusing.aikars.flags=https://mcflags.emc.gs -Daikars.new.flags=true -jar server.jar nogui
...
#                 cd $DIR
#                 tmux new -d -s watcher $DIR/mcontrol watcher
#         fi
...
#                 $DIR/bin/mcrcon -p $RCONPASS -w 5 "say Server is shutting down in 10 seconds!" save-all stop 2>/dev/null
...
#                 tmux has-session -t minecraft 2>/dev/null
#                 if [[ $? != 0 && $STATUS == "FREE" ]]; then
#                         echo "[ $(tput setaf 2)SUCCESS$(tput sgr 0) ] Server halted successfully"
#                 else
#                         echo "[  $(tput setaf 1)ERROR$(tput sgr 0)  ] Could not halt server. Waiting 5 seconds and trying again."
#                         sleep 5
...
#         if [ $? != 0 ]; then
#                 echo "[  $(tput setaf 1)ERROR$(tput sgr 0)  ] Server is not running"
#                 $DIR/mcontrol nw_start
#                 sleep 120
...
#                         exit
#                 else
#                         echo "[ $(tput setaf 2)SUCCESS$(tput sgr 0) ] Server running."
#                         sleep 60
...
#                 $DIR/bin/mcrcon -p $RCONPASS "say Running automated backup, server going read-only" save-off save-all

#                 echo "[ $(tput setaf 2)SUCCESS$(tput sgr 0) ] World save complete!"

#                 echo "[  $(tput setaf 3).....$(tput sgr 0)  ] Compressing backup"
                
#                 cd $DIR/serverfiles

#                 tar -cpvzf $DIR/backups/world$(date +"%F@%R").tar.gz world
#                 tar -cpvzf $DIR/backups/world_nether$(date +"%F@%R").tar.gz world_nether
#                 tar -cpvzf $DIR/backups/world_the_end$(date +"%F@%R").tar.gz world_the_end
                
#                 echo "[ $(tput setaf 2)SUCCESS$(tput sgr 0) ] Backup compressed"

#                 echo "[  $(tput setaf 3).....$(tput sgr 0)  ] Removing old backups"

#                 find $DIR/backups/* -mtime +7 -exec rm -rf {} \;
...
#                 $DIR/bin/mcrcon -p $RCONPASS save-on save-all "say Backup completed"
